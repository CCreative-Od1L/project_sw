...
- **挂起解锁也必须失效**:密码 KDF 或生物提示正在进行时切后台,会话真相源立即作废当前 `SessionUnlockAttempt` 并清除 repository 的 MVK、摘要与 KDF 投影;晚到结果不得重新解锁、恢复摘要或触发后续会话的生物失效。
...
- **显式活动状态**:迁移发送、迁移接收与忘码恢复由 `UnlockedSession` 的 `SessionActivity` 表达,并由唯一 lease 防止旧回调干扰后续同类活动;活动期间仅暂停 idle timer,结束后开启新的完整空闲窗口。活动标志不得由页面另存。
...
- **超时抑制边界**:忘码恢复进行中以 `SessionActivity.passwordRecovery` 显式发布并抑制 idle timeout,但不抑制切后台立即锁定、手动锁定或生物失效;锁定后的异步完成回调不得恢复解锁态。
...
- **死锁擦除**(锁定态,主密码与生物均不可用时逃生):不要求主密码,为隐式应急通道,带四重摩擦:
  - **入口隐式不主动显示**:锁定态默认不暴露;当前固定手势为 **3 秒内连续点击锁屏标志 7 次**;
  - **强告知**:"此操作将永久删除所有密码,不可恢复";
  - **二次显式确认**:要求准确键入固定确认词 `DELETE`;
  - **延迟执行 + 倒计时**:点击后启动 **10 秒**倒计时,期间可取消,超时才执行。
