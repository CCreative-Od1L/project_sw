...
| **内层** | 单条目明文 `{name,url,username,password,notes}`(加密前输入,字段规格见 [VaultEntry 规格](specs/vault_entry.md)) | 明文,加密后变 blob | **JSON**(`plaintext_format_id` 逐条标识) |
...
- **覆盖范围**:`password` 与 `custom_fields(secret=true)`(见 [VaultEntry 规格](specs/vault_entry.md))复用同一剪贴板路径与同一清除超时;生成器直接输出(锁定态或解锁态)复制同样走此超时路径。复制非敏感字段(如 username)不触发超时清除(短按需复制即可)。
...
- **自定义字段卫生**:`custom_fields` 中 `secret=true` 的字段与 password 同级——不入可搜、不入列表展示,仅详情页按需解密;`secret=false` 的字段可搜可展示(字段规格见 [VaultEntry 规格](specs/vault_entry.md))。
...
- **可记忆密码引导**:建库时引导选强且可记的密码,可借生成器 pronounceable 模式(见 [密码生成器规格](specs/password_generator.md))生成可记密码,降低遗忘概率。
...
- **额外摩擦**(进入恢复流程时):
  ...
  - 须设置新主密码并走强度评估(见 [密码生成器规格](specs/password_generator.md)),不接受弱密码。
...
生成器完整规格(模式、字符集、长度、强度评估口径)见 [密码生成器规格](specs/password_generator.md)。
