...
| 迭代 `t` | **3** | 与 m=64MiB 配合,目标派生耗时 250–400ms；v0.1 在 Xiaomi M2007J17C / Android 12 上实测中位数为 280ms,验证该设备基线上的设计假设 |
...
2026-08-02 在 Xiaomi M2007J17C（Android 12、8 核）通过
`integration_test/argon2id_calibration_test.dart` 对默认档位各运行 3 次,
记录中位数。报告不包含主密码、盐或派生密钥。
...
所有档位均在 1 秒选择阈值内,因此首次建库会持久化最高档
`m=96MiB,t=4,p=1`。64MiB/t=3 的 280ms 中位数落在原 250–400ms
假设内,所以保留该范围。该数据仅是此设备的基线;每台设备首次建库仍须
重新运行自适应基准。
...
| 剪贴板泄漏 | 敏感复制路径最小化暴露、禁止日志记录、遵循系统剪贴板隐私能力;不保证 Android 各厂商或第三方输入法的剪贴板能够被应用清除,也不把应用侧固定时限自动清除作为安全保证(见 data_hygiene.md) |
